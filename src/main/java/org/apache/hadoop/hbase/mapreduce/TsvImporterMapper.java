...
public class TsvImporterMapper
    ...
    protected void doSetup(Context context) {
        ...
        skipBadLines = context.getConfiguration()
            .getBoolean(ImportTsv.SKIP_LINES_CONF_KEY, true);
     ...
     */
    @Override public void map(LongWritable offset, Text value, Context context)
        throws IOException {
        byte[] lineBytes = value.getBytes();

        try {
            ImportTsv.TsvParser.ParsedLine parsed =
                parser.parse(lineBytes, value.getLength());
            ImmutableBytesWritable rowKey =
                new ImmutableBytesWritable(lineBytes, parsed.getRowKeyOffset(),
    ...
    protected void populatePut(byte[] lineBytes,
        ImportTsv.TsvParser.ParsedLine parsed, Put put, int i)
        ...
        if (hfileOutPath == null) {
            ...
            if (cellVisibilityExpr != null) {
                // We won't be validating the expression here. The Visibility
                // CP will do
                // the validation
            ...
            // Add TTL directly to the KV so we can vary them when packing
            // more than one KV
            // into puts
            if (ttl > 0) {
